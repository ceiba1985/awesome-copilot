...
### Managing Issues, Pull Requests, and Sessions

Recent releases have expanded how you triage and manage work directly from the app:

- **Edit issues and pull requests in place**: You can edit issue and pull request titles and descriptions, and edit, delete, or hide comments—without leaving the app.
- **"Fix with instructions"**: Pull request fix buttons now include a chevron with a "Fix with instructions" option, letting you add guidance before the agent runs the fix.
- **Chats pill**: A Chats pill above the composer shows unread and input-required status across your side chats, with quick navigation between them.
- **Session sorting**: The Sessions sidebar supports Recent, Created, Name, and classic None sorting, with your chosen order saved across restarts.
- **Windows 11 taskbar integration**: Running sessions show live hover status cards in the Windows 11 taskbar, so you can check progress without switching to the app window.
- **Generated artifacts**: Markdown files generated by an agent now open in the Files tab alongside repository files, with a switcher between the two and an option to promote a generated artifact into the repository.
