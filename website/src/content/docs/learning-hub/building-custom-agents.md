...
> **New (v1.0.83+)**: `model` can list **several models**, tried in order until one is available to you—useful as a fallback chain when a preferred model is rate-limited or temporarily unavailable:
>
> ```yaml
> ---
> name: 'Security Reviewer'
...
> ```
>
> Add `model-policy: required` to keep users from switching away from the listed models mid-session, ensuring the agent always runs on one of its declared models.
...
| Storytelling/creative reasoning experiments | claude-fable-5.1 *(v1.0.83+)* |
| Fast, lightweight reasoning | Gemini 3.7 Flash *(v1.0.81+)* |
