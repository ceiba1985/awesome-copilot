...
**Model fallback lists** *(v1.0.83+)*: `model` can also be an array of models, tried in order until one is available to you. This is useful when your preferred model is temporarily rate-limited, deprecated, or not enabled for your account:
...
Add `model-policy: required` to keep any user-initiated model changes restricted to that same list, preventing someone from switching the agent to an unapproved model mid-session:

```yaml
---
name: 'Security Reviewer'
description: 'Expert security auditor'
model: ['Claude Sonnet 5', 'Claude Sonnet 4']
