...
**Model fallback lists** *(v1.0.83+)*: `model` can also be a list of several models instead of a single value. Copilot tries each entry in order and uses the first one available to you—useful when an agent's preferred model may be rate-limited, temporarily unavailable, or not enabled on your account:
...
Add `model-policy: required` alongside a model list to keep the session pinned to models from that list even if the user tries to switch to something else mid-session with `/model`:
