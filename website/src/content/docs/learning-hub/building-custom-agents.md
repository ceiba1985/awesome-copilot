...
*(v1.0.83+)* You can list several models in `model`, tried in order until one is available to you — useful when your preferred model is temporarily unavailable or not enabled for your account:
...
Add `model-policy: required` to keep any later model changes (made by the user or another agent) restricted to that same fallback list, instead of allowing an arbitrary model swap.
