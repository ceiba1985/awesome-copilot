...
**Model fallback lists** *(v1.0.83+)*: `model` can be an array of several model names instead of a single string. Copilot CLI tries each entry in order and uses the first one that's actually available to the current user, which is useful when an agent is shared across a team with different model access. Pair it with `model-policy: required` to keep any in-session model changes (via `/model`) restricted to that same list, instead of letting the user switch to an unrelated model:
...
| Alternative frontier model, general reasoning | claude-fable-5.1 *(v1.0.83+)* |
