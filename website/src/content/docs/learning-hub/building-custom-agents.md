...
**Model fallback lists** *(v1.0.83+)*: `model` can also be a list of several models, tried in order until one is available to you:

```yaml
---
name: 'Release Notes Writer'
description: 'Drafts release notes from merged pull requests'
model: ['Claude Opus 5', 'Claude Sonnet 4', 'GPT-5']
---
```

This keeps an agent usable even if your preferred model is temporarily rate-limited or not enabled for your account. Add `model-policy: required` to keep any in-session model changes restricted to that same list, preventing a user from switching the agent to an unlisted model mid-conversation:
