...
- **OAuth**: MCP servers can use the OAuth flow to authenticate with external services. The CLI handles the browser redirect and token storage automatically. This also works when running in ACP (Agent Coordination Protocol) mode. *(v1.0.83+)* Copilot CLI also supports **Client ID Metadata Document (CIMD)** for MCP OAuth sign-in, an emerging standard that lets a client publish its metadata at a URL instead of requiring pre-registration with each authorization server.
