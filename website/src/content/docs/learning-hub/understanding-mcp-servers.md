...
- **Client ID Metadata Document (CIMD)** *(v1.0.83+)*: MCP OAuth sign-in supports CIMD, letting an MCP server discover client configuration from a hosted metadata document instead of requiring a manually registered client ID. This simplifies OAuth setup for enterprise MCP servers that support the CIMD standard.
