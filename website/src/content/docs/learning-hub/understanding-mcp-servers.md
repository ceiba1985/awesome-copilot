...
- **Client ID Metadata Document (CIMD)** *(v1.0.83+)*: Copilot CLI supports CIMD for MCP OAuth sign-in, an alternative to Dynamic Client Registration where the OAuth client identifies itself via a metadata document URL instead of registering with the authorization server ahead of time. This broadens compatibility with MCP servers and identity providers that support the CIMD approach.
