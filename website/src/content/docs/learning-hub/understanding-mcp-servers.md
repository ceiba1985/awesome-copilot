...
> **New (v1.0.83+)**: Copilot CLI supports **Client ID Metadata Document (CIMD)** for MCP OAuth sign-in. Instead of relying solely on Dynamic Client Registration, an MCP server can publish its OAuth client metadata at a well-known URL, letting the CLI verify the client's identity during authentication without a separate registration step.
