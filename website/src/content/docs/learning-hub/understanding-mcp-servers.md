...
> **New (v1.0.83+)**: Copilot CLI supports **Client ID Metadata Document (CIMD)** for MCP OAuth sign-in. Instead of registering a static client ID with each MCP server's authorization server, the CLI can publish a metadata document describing itself and have servers fetch it dynamically — simplifying OAuth setup for MCP servers that support the CIMD draft spec and reducing per-server registration overhead.
