...
- **Client ID Metadata Document (CIMD)** *(v1.0.83+)*: MCP servers that support CIMD can sign in without requiring a manually registered OAuth client — the server fetches client metadata from a hosted document instead. This simplifies OAuth setup for MCP servers that adopt the CIMD pattern, removing the need for pre-registration steps.
