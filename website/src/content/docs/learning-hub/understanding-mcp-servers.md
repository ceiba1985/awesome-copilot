...
> **New (v1.0.83+)**: Copilot CLI supports **Client ID Metadata Document (CIMD)** for MCP OAuth sign-in. Instead of relying only on Dynamic Client Registration, an MCP server can point to a hosted client metadata document that describes the CLI's OAuth client — useful for enterprise identity providers that don't support DCR but do support CIMD-based client identification.
