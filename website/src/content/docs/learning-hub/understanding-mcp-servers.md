...
- **Microsoft Entra ID (Azure AD)**: MCP servers that authenticate via Microsoft Entra ID are fully supported. Once you complete the initial login, the CLI caches the authentication and **will not show the consent screen on subsequent connections** — you authenticate once per session rather than every time the server reconnects. On Windows, sign-in can also go through the OS authentication broker (WAM), which usually completes with no prompt at all.
- **Client ID Metadata Document (CIMD)** *(v1.0.83+)*: MCP servers that support CIMD for OAuth sign-in let the CLI register itself using a hosted client metadata document instead of manual dynamic client registration, simplifying setup for servers that support the standard.
