...
**Namespaced and ignored skill directories** *(v1.0.88+)*: The CLI now supports namespaced custom skills and ignored skill directories during discovery, so you can scope skill names to a plugin or source (avoiding collisions between similarly named skills from different providers) and exclude specific directories from skill discovery entirely.
