...
> **Sandbox network isolation tightened (v1.0.83+)**: On macOS and Linux, sandboxed commands can no longer reach services running on your own machine, including a server the sandboxed command itself starts on `127.0.0.1` — so test suites that bind a local port will fail unless you turn on **Allow local network** in `/sandbox`. Linux sandboxing (both the default filesystem sandbox and the newer proxy-restricted egress mode) now requires `slirp4netns`, `nsenter`, `iptables`, `ip6tables`, `iptables-restore`, and `ip6tables-restore` on `PATH` (plus `util-linux` 2.35+ and `/dev/net/tun` access for proxy mode); install them if sandboxed commands start failing to launch on Linux.

> **Managed org sign-in pinning and MCP OAuth (v1.0.83+)**: Enterprise admins can restrict sign-in to approved GitHub organizations with the `forceLoginOrgs` managed setting. Separately, MCP OAuth sign-in now supports **Client ID Metadata Document (CIMD)**, an emerging standard for dynamic OAuth client registration without pre-registering a client ID with each server.
