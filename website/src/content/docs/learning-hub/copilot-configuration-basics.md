...
**Sandbox network restrictions** *(v1.0.83+)*: On macOS and Linux, sandboxed commands can no longer reach services running on your own machine, including a server the command itself starts on `127.0.0.1` — so test suites that bind a local port will fail inside the sandbox by default. Turn on **Allow local network** in `/sandbox` to restore access to localhost services.

> **New Linux sandbox dependencies (v1.0.83+)**: Linux sandboxing now requires `slirp4netns`, `nsenter`, `iptables`, `ip6tables`, `iptables-restore`, and `ip6tables-restore` on `PATH`. If sandboxed commands suddenly fail to launch after upgrading, install these packages. Proxy-restricted egress mode additionally requires `util-linux` 2.35+ and `/dev/net/tun` access, since Linux sandboxes now restrict network egress to the configured proxy when one is set.
